 ...
 */
class TBGLSSOAuthentication extends TBGModule {

    protected $_longname = 'SSO Authentication';
    protected $_description = 'Remote authentication allows you to authenticate users using a locally hosted script.';
    protected $_module_config_title = 'SSO Authentication';
    ...
    {
	return TBGContext::getModule('auth_sso');
    }
	...
	$this->addRoute('logout', '/logout','login');

	/*$this->addRoute('sso_test', '/test/sso', 'testConnection');
	$this->addRoute('sso_prune', '/configure/module/auth_sso/prune', 'pruneUsers');
	$this->addRoute('sso_import', '/configure/module/auth_sso/import', 'importUsers');*/
    }

    protected function _install($scope)
    {
	$this->saveSetting('sharedkey', TBGUser::createPassword(36));
    }
    ...
    {
	return TBGContext::getRouting()->generate('sso_authentication_index');
    }

    public function postConfigSettings(TBGRequest $request)
    {
	$settings = array('remotelogouturl', 'remoteloginurl');
	...
	$connection = sso_connect($host);
	sso_set_option($connection, SSO_OPT_PROTOCOL_VERSION, 3);
	sso_set_option($connection, SSO_OPT_REFERRALS, 0);
    ...
    {
/*	$bind = sso_bind($connection, $lduser, $ldpass);

	if (!$bind)
	{
	    sso_unbind($connection);
	    TBGLogging::log('bind failed: ' . sso_error($connection), 'sso', TBGLogging::LEVEL_FATAL);
	    throw new Exception(TBGContext::geti18n()->__('Failed to bind: ') . sso_error($connection));
	}*/
    }
...
//
//	$user_class = TBGContext::getModule('auth_sso')->getSetting('u_type');
//	$group_class = TBGContext::getModule('auth_sso')->getSetting('g_type');
//
//	$email = null;
//
//	/*
//	 * Do the SSO check here.
//	 * 
//	 * If a connection error or something, throw an exception and log
//	 * 
//	 * If we can, set $mail and $realname to correct values from SSO
//	 * otherwise don't touch those variables.
//	 * 
//	 * To log do:
//	 * TBGLogging::log('error goes here', 'sso', TBGLogging::LEVEL_FATAL);
//	 */
//	try
//	{
//	    /*
...
//	    $filter = '(&(objectClass=' . TBGSSOAuthentication::getModule()->escape($user_class) . ')(' . $username_attr . '=' . $this->escape($username) . '))';
//
//	    $results = sso_search($connection, $base_dn, $filter, $fields);
//
//	    if (!$results)
//	    {
//		TBGLogging::log('failed to search for user: ' . sso_error($connection), 'sso', TBGLogging::LEVEL_FATAL);
//		throw new Exception(TBGContext::geti18n()->__('Search failed: ') . sso_error($connection));
//	    }
//
//	    $data = sso_get_entries($connection, $results);
//
//	    // User does not exist
//	    if ($data['count'] == 0)
//	    {
//		TBGLogging::log('could not find user ' . $username . ', class ' . $user_class . ', attribute ' . $username_attr, 'sso', TBGLogging::LEVEL_FATAL);
...
//	    if ($data['count'] > 1)
//	    {
//		TBGLogging::log('too many users for ' . $username . ', class ' . $user_class . ', attribute ' . $username_attr, 'sso', TBGLogging::LEVEL_FATAL);
...
//		    $filter2 = '(&(objectClass=' . TBGSSOAuthentication::getModule()->escape($group_class) . ')(cn=' . $this->escape($group) . '))';
//
//		    $results2 = sso_search($connection, $base_dn, $filter2, $fields2);
//
//		    if (!$results2)
//		    {
//			TBGLogging::log('failed to search for user after binding: ' . sso_error($connection), 'sso', TBGLogging::LEVEL_FATAL);
//			throw new Exception(TBGContext::geti18n()->__('Search failed ') . sso_error($connection));
//		    }
//
//		    $data2 = sso_get_entries($connection, $results2);
//
//		    if ($data2['count'] != 1)
//		    {
//			continue;
