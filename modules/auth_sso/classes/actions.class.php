...
/**
 * actions for the sso_authentication module
 */
class auth_ssoActions extends TBGAction {
    ...
    {
	$validgroups = TBGContext::getModule('auth_sso')->getSetting('groups');
	$base_dn = TBGContext::getModule('auth_sso')->getSetting('b_dn');
	$groups_members_attr = TBGContext::getModule('auth_sso')->getSetting('g_attr');
	$group_class = TBGContext::getModule('auth_sso')->getSetting('g_type');

	try
	{
	    $connection = TBGContext::getModule('auth_sso')->connect();
	...
	} catch (Exception $e)
	{
	    ...
	    $this->forward(TBGContext::getRouting()->generate('configure_module', array('config_module' => 'auth_sso')));
	}
	...
	$validgroups = TBGContext::getModule('auth_sso')->getSetting('groups');
	$base_dn = TBGContext::getModule('auth_sso')->getSetting('b_dn');
	$dn_attr = TBGContext::getModule('auth_sso')->getSetting('dn_attr');
	$username_attr = TBGContext::getModule('auth_sso')->getSetting('u_attr');
	$fullname_attr = TBGContext::getModule('auth_sso')->getSetting('f_attr');
	$email_attr = TBGContext::getModule('auth_sso')->getSetting('e_attr');
	$groups_members_attr = TBGContext::getModule('auth_sso')->getSetting('g_attr');
	...
	{
	    $connection = TBGContext::getModule('auth_sso')->connect();
	    TBGContext::getModule('auth_sso')->bind($connection, TBGContext::getModule('auth_sso')->getSetting('control_user'), TBGContext::getModule('auth_sso')->getSetting('control_pass'));
	...
	try
	{
	    ...
	    if (!$results)
	    {
		TBGLogging::log('failed to search for users: ' . sso_error($connection), 'sso', TBGLogging::LEVEL_FATAL);
		throw new Exception(TBGContext::geti18n()->__('Search failed: ') . sso_error($connection));
	    }
